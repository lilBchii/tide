...
use iced::widget::{button, column, container, opaque, row, text, Scrollable, Space, Text};
...
impl DebugZone {
    ...
    fn errors(&self) -> String { format!("{}", self.typst_errors)}
            ...
            container(
                column![
                    row![
                        ...
                        button(Text::new("X").shaping(Shaping::Advanced)).on_press(Message::HideErrors)
                        .style(validate_button),
                    ],
                    Scrollable::new(text(self.errors())
                        .shaping(Shaping::Advanced)
                        .size(18))
                ]
            ).style(debug_container_style).width(Fill).height(250)
        )
    }
}
