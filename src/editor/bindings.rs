...
pub fn bindings(key_press: KeyPress) -> Option<Binding<editing::Message>> {
    match key_press.key.as_ref() {
        ...
        Key::Character("s") if key_press.modifiers.command() => {
            Some(Binding::Sequence(vec![
                Binding::Custom(editing::Message::ToolBar(
                    toolbar::Message::ForcePreview,
                )),
                Binding::Custom(editing::Message::ToolBar(toolbar::Message::SaveFile(
                    false,
                ))),
            ]))
        }
        ...
        Key::Character("o") if key_press.modifiers.command() => {
            Some(Binding::Sequence(vec![
